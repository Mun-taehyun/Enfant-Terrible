...
import { AUTH_LOGIN_PATH, AUTH_PATH, MAIN_PATH, USER_PATH } from "@/constant/user/route.index";
...
export const useAuth = () => {
    ...
    //서버상태 : 내 정보 불러오기 
    const {data: myInfo, error : myInfoError , isLoading : isInfoLoading , refetch } = userQueries.useMe();
    ...
    const onSignUpClick = () => {
        ...
        if (isTelValid && hasZip && hasAddr) {
            signUp(
                ...
                {
                    onSuccess: () => {navigate(AUTH_PATH() + AUTH_LOGIN_PATH()); resetForm(); return;},
                    onError: (error:Error) => {alert(error.message)}
                }
            );
        }
