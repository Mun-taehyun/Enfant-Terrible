...
import type { EmailCertificationRequestDto } from '@/apis/user/request/auth';
...
/*        const signUpResponse = (responseBody: SignUpResponseDto | ResponseDto | null) => {
    ...
    // ========================= 회원가입 카드 이벤트핸들러 ==============================
        ...
        const onNextButtonClickHandler1 = () => {
            ...
            if(!isEmailPattern || !isCheckedPassword || !isEqualPassword || !isNamePattern ) return; 
            // 이메일주소 , 비번8자 ,비번확인, 이름 불일치 =>넘어갈 수 없다. 
            setPage(2);
        }//1페이지 기준 .. 
        ...
        const onSignUpButtonClickHandler = () => {
            ...
            if(!isEmailPattern || !isCheckedPassword || !isEqualPassword || !isNamePattern ) {
                // 이메일주소 , 비번8자 ,비번확인 불일치 =>넘어갈 수 없다. 
                setPage(1);
                return;
...
*/
    ...
    // 이펙트 : 페이지가 변경될 때마다 실행
        useEffect(() => {
            ...
            if(page === 3) { //넘어가도 되는 지 (추가번호 입력칸 로딩여부..)
                if(!추가입력.current) return;
                추가입력.current.focus();
            }
        }, [page])
            ...
            <div className='auth-card'>
                <div className='auth-card-box'>
                    <div className='auth-card-top'>
                        ...
                        <InputBox ref={telRef} label='핸드폰 번호*' type='text' placeholder='핸드폰 반호를 입력해주세요.' value={tel} onChange={onTelChangeHandler} error={isTelError} message={telErrorMessage} onkeyDown={onTelKeyDownHandler} />
                        <InputBox ref={addressRef} label='주소*' type='text' placeholder='우편번호 찾기' value={address} onChange={onAddressChangeHandler} error={isAddressError} message={addressErrorMessage} icon='expend-right-light-icon' onButtonClick={onAddressButtonClickHandler} onkeyDown={onAddressKeyDownHandler}/>
                    ...
                    <div className='auth-card-bottom'>
                        {page === 1 &&( //버튼 1개
                        <div className='black-large-full-button' onClick={onNextButtonClickHandler1}>{'다음 단계'}</div>
                        )}
                        {page === 2 &&( //버튼 1개
                        <div className='black-large-full-button' onClick={onNextButtonClickHandler2}>{'다음 단계'}</div>
                        )}
                        {page === 3 &&( //체크박스 + 글씨 + 클릭이벤트  , 클릭이벤트 구조 
                        <>
