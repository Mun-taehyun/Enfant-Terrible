...
module.exports = function registerCategoriesRoutes(server, router, common) {
  ...
  function applyActiveFromBody(row, raw) {
    if (raw === undefined) return { changed: false };

    const s = String(raw).trim();
    if (s === "ACTIVE") { row.is_active = 1; return { changed: true }; }
    if (s === "INACTIVE") { row.is_active = 0; return { changed: true }; }
    if (s === "Y") { row.is_active = 1; return { changed: true }; }
    if (s === "N") { row.is_active = 0; return { changed: true }; }

    return { changed: false, error: "활성 값은 'ACTIVE'|'INACTIVE' 또는 'Y'|'N' 이어야 합니다." };
  }
  ...
  server.patch("/api/admin/categories/:categoryId(\\d+)", (req, res) => {
    const list = dbCategories();
    if (!list) return res.status(500).json(fail("db.json에 categories 배열이 없습니다."));
    ...
    const nextIsActiveRaw = req.body?.isActive;
    ...
    if (nextStatusRaw !== undefined) {
      const r = applyActiveFromBody(list[idx], nextStatusRaw);
      if (r.error) return res.status(400).json(fail(r.error));
    } else if (nextIsActiveRaw !== undefined) {
      const r = applyActiveFromBody(list[idx], nextIsActiveRaw);
      if (r.error) return res.status(400).json(fail(r.error));
    }
  ...
  server.patch("/api/admin/categories/:categoryId(\\d+)/active", (req, res) => {
  ...
  // PATCH /api/admin/categories/:categoryId/sort-order?sortOrder=...
  server.patch("/api/admin/categories/:categoryId(\\d+)/sort-order", (req, res) => {
    const n = Number(req.query?.sortOrder);
    if (Number.isNaN(n) || n <= 0) return res.status(400).json(fail("sortOrder는 1 이상의 숫자여야 합니다."));
  ...
  server.patch("/api/admin/categories/:categoryId(\\d+)/move", handleMove);
  server.patch("/api/admin/categories/:categoryId(\\d+)/parent", handleMove);

  // DELETE /api/admin/categories/:categoryId
  server.delete("/api/admin/categories/:categoryId(\\d+)", (req, res) => {
    const list = dbCategories();
    if (!list) return res.status(500).json(fail("db.json에 categories 배열이 없습니다."));
