...
public class MailService {
  ...
  /**
   * 비밀번호 재설정 이메일 인증 메일 발송
   */
  public void sendPasswordResetVerification(String toEmail, String code) {
    ...
    try {
      ...
      helper.setTo(toEmail);
      helper.setSubject("[EnfantTerrible] 비밀번호 재설정 인증 안내");

      String html = buildPasswordResetVerificationHtml(code);
   ...
   * 비밀번호 재설정 인증 메일 HTML
   */
  private String buildPasswordResetVerificationHtml(String code) {

    return """
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto;">
        <h2>비밀번호 재설정 안내</h2>
        <p>아래 인증 코드를 비밀번호 재설정 화면에 입력해 주세요.</p>
